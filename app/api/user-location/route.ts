...
export async function GET() {
  try {
    ...
    await clientPromise;
    const user = await User.findById(session.user.id).select('selectedLocation').lean();

    if (!user) {
      return NextResponse.json({ error: 'User not found' }, { status: 404 });
    }

    return NextResponse.json({ selectedLocation: user.selectedLocation ?? null });
  } catch (error) {

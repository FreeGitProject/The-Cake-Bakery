...
export default function Header() {
  ...
  const navItems = ['Home', 'News', 'About', 'Favorites', 'Location'];
      ...
      <div className="flex items-center justify-between">
          ...
          {session ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="p-0 hover:bg-[#FFF5E4] transition-colors duration-300">
              ...
              <DropdownMenuContent align="end" className="w-48">
                ...
                <DropdownMenuItem onSelect={() => signOut()} className="hover:bg-[#FFF5E4]">
                  Sign Out
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        >
          <div className="w-6 h-6 relative transform transition-all duration-300">
            <span className={`absolute h-0.5 w-full bg-[#FF9494] transform transition-all duration-300 ${isOpen ? 'rotate-45 translate-y-2.5' : ''}`} />
            <span className={`absolute h-0.5 w-full bg-[#FF9494] transform transition-all duration-300 translate-y-2 ${isOpen ? 'opacity-0' : ''}`} />
            <span className={`absolute h-0.5 w-full bg-[#FF9494] transform transition-all duration-300 translate-y-4 ${isOpen ? '-rotate-45 -translate-y-2.5' : ''}`} />
          </div>
        </button>
      </div>

      {/* Mobile Menu */}
      <div className={`md:hidden transform transition-all duration-300 ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>

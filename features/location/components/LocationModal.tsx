...
export default function DeliveryCityModal() {
  const { currentLocation, setCurrentLocation, isLocationModalOpen, setIsLocationModalOpen } =
    useLocation();
  ...
  const getCityIcon = (city: City) => {
    switch (city.size) {
      case 'metro':
        return (
          <Building2 className={`h-5 w-5 ${city.isAvailable ? 'text-primary' : 'text-gray-400'}`} />
        );
      case 'large':
        return (
          <Building2 className={`h-5 w-5 ${city.isAvailable ? 'text-primary' : 'text-gray-400'}`} />
      ...
      default:
        return (
          <Home className={`h-5 w-5 ${city.isAvailable ? 'text-primary' : 'text-gray-400'}`} />
        );
    }
  };
    ...
    >
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="bg-gray-100 dark:bg-gray-700 rounded-full p-2">{getCityIcon(city)}</div>
          <div>
            <p className="font-medium">
              {city?.name?.toLocaleUpperCase()}, {city?.state?.toLocaleUpperCase()}
            </p>
          </div>
        </div>
      ...
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader className="flex flex-row items-center gap-3 pb-2">
          ...
          <div>
            ...
            <DialogDescription>Choose a city where you want your order delivered</DialogDescription>
          </div>
        </DialogHeader>
        ...
        {detectedCity && !currentLocation && (
          <div className="mb-4 p-3 bg-primary/5 rounded-lg border border-primary/20">
            <div className="flex items-start gap-3">
              <div className="bg-primary/10 p-2 rounded-full mt-1">{getCityIcon(detectedCity)}</div>
              <div>
                ...
                {detectedCity.isAvailable ? (
                  <Button onClick={() => handleCitySelect(detectedCity)} className="mt-1" size="sm">
          ...
          <TabsContent value="all" className="mt-4">
            <div className="max-h-[350px] overflow-y-auto pr-1">
              {isLoading ? (
                Array(5)
                  .fill(0)
                  .map((_, i) => <CityCardSkeleton key={i} />)
              ) : filteredCities.length > 0 ? (
                filteredCities.map((city) => (
                  <CityCard key={`${city.name}-${city.state}`} city={city} />
                ))
              ) : (
        ...
        <div className="text-center mt-4 flex items-center justify-center gap-1 text-xs text-gray-500">
          ...
          <span>
            Not all cities have delivery service available yet. We&apos;re expanding our delivery
            areas.
          </span>
        </div>
      </DialogContent>

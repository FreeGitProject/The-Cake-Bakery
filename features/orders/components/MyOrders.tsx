...
const PaymentStatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span
      className={`flex items-center px-3 py-1 rounded-full text-sm font-medium ${color} ${textColor} shadow-sm`}
    >
      {icon}
      {status}
    </span>
    ...
    <div className="max-w-7xl mx-auto space-y-6 px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
        {orders.map((order) => (
          <Card
            key={order._id}
            className="overflow-hidden hover:shadow-xl transition-shadow duration-300 bg-white backdrop-blur-lg"
          >
                  ...
                  >
                    ...
                    <div className="flex-grow space-y-1">
                      ...
                      <div className="text-sm text-gray-500">
                        <p>
                          {item.caketype === 'cake' ? `${item.weight}kg` : `${item.weight} pieces`}
                        </p>

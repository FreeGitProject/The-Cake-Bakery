  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch(
        `/api/admin/orders?page=${currentPage}&status=${statusFilter}&search=${searchTerm}`
      );
      if (response.ok) {
    ...
    <div className="space-y-6 p-6 max-w-7xl mx-auto">
                    ...
                    <div>
                      ...
                      {order.isGift && (
                        <div className="flex items-cente">
                          <p className="text-gray-600">
                            Gift Message:{' '}
                            <span className="font-semibold">{order.giftMessage} </span>
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <TableBody>
                    {order.orderItems.map((item, index) => (
                      <TableRow key={index} className="hover:bg-gray-50">
                        <TableCell>
                          <Link
                            href={`/cakes/${item.productId}`}
                            className="block w-12 h-12 relative rounded-lg overflow-hidden"
                          >
                            <Image
      ...
      <div className="flex justify-center mt-8">
        <div className="flex items-center space-x-2">
          ...
          <span className="ml-4 text-sm text-muted-foreground">Total Records: {totalRecords}</span>
        </div>
      </div>
    </div>
  );

      ...
      if (response.ok) {
        ...
        if (currentStats && prevData.overallStats) {
          ...
          setComparisonStats({
            ...
            ordersChange: calculateChange(currentStats.totalOrders, prevStats.totalOrders),
  ...
  const exportData = (dataType: string) => {
    ...
    if (dataType === 'revenue') {
      ...
      stats.topCustomers.forEach((item) => {
        rows.push([item.username, item.email, item.totalOrders, item.totalSpent]);
      });
    }
              ...
              <CardContent>
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-2xl font-bold">
                      {(
                        (stats.overallStats.completed / stats.overallStats.totalOrders) *
                        100
                      ).toFixed(1)}
                      %
                    </div>
                    <div className="flex items-center mt-1">
                      <div className="text-xs text-muted-foreground">
                        {stats?.overallStats?.completed} of {stats?.overallStats?.totalOrders}
                      </div>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Button variant="ghost" size="icon" onClick={() => exportData('revenue')}>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => showDetails('status', stats.orderStatusDistribution)}
                >
                  Details
                </Button>
              </CardHeader>
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-white p-4 border rounded-md shadow-md">
                              ...
                              <p>
                                Percentage:{' '}
                                {((data.count / stats.overallStats.totalOrders) * 100).toFixed(1)}%
                              </p>
                            </div>
                          );
                        }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Button variant="ghost" size="icon" onClick={() => exportData('bestsellers')}>
                  <Download className="h-4 w-4" />
                </Button>
              </CardHeader>
              <CardContent className="h-[400px]">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => showDetails('products', stats.bestSellingCakes)}
                >
                  Details
                </Button>
              </CardHeader>
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-white p-4 border rounded-md shadow-md">
                              ...
                              <p>
                                Avg Price: ₹{(data.totalRevenue / data.totalQuantity).toFixed(0)}
                              </p>
                            </div>
                          );
                        }
                      ...
                      shape={(props: { payload?: any; cx?: any; cy?: any }) => {
                        ...
                        const scaledSize = Math.max(
                          ...
                          Math.min(
                            60,
                            (totalRevenue /
                              Math.max(...stats.bestSellingCakes.map((c) => c.totalRevenue))) *
                              size
                          )
                        );
                        return (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Button variant="ghost" size="icon" onClick={() => exportData('customers')}>
                  <Download className="h-4 w-4" />
                </Button>
              </CardHeader>
              <CardContent>
                  ...
                  {stats.topCustomers.slice(0, 5).map((customer) => (
                    <div key={customer._id} className="flex items-center">
                      ...
                      <div className="space-y-1 flex-1">
                        <p className="text-sm font-medium leading-none">{customer.username}</p>
                ...
                {stats.topCustomers.length > 5 && (
                  <div className="mt-4 pt-4 border-t">
                    <Button
                      ...
                      onClick={() => showDetails('customers', stats.topCustomers)}
                    >
                      View All Customers
                    </Button>
                  </div>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div>
                  ...
                  <CardDescription>Orders and spending patterns</CardDescription>
                </div>
              </CardHeader>
              <CardContent className="h-[350px]">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart outerRadius={90} data={stats.topCustomers.slice(0, 5)}>
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-white p-4 border rounded-md shadow-md">
                              ...
                              <p>
                                {label}:{' '}
                                {metric === 'totalSpent' ? `₹${value.toLocaleString()}` : value}
                              </p>
                              <p>
                                Avg Order: ₹
                ...
                <ScatterChart margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                      ...
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div className="bg-white p-4 border rounded-md shadow-md">
                            ...
                            <p>
                              Avg Order: ₹{(customer.totalSpent / customer.totalOrders).toFixed(0)}
                            </p>
                          </div>
                        );
                      }
                  ...
                  <Scatter
                    ...
                    data={stats.topCustomers.map((customer) => ({
                      ...customer,
                      avgOrderValue: (customer.totalSpent / customer.totalOrders).toFixed(0),
                    }))}
                    fill="#f72585"
                    shape={(props: { payload?: any; cx?: any; cy?: any }) => {
                      const { cx, cy } = props;
                      ...
                      const maxSpent = Math.max(...stats.topCustomers.map((c) => c.totalSpent));
              ...
              <TableBody>
                {detailData.map((item: any) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell>
                      {((item.count / stats.overallStats.totalOrders) * 100).toFixed(1)}%
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
              ...
              <TableBody>
                {detailData.map((item: any) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell>₹{(item.totalRevenue / item.totalQuantity).toFixed(0)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
              ...
              <TableBody>
                {detailData.map((item: any) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell>₹{(item.totalSpent / item.totalOrders).toFixed(0)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>

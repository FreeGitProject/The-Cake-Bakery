...
import { useHomeData } from '@/features/content/hooks/useContentData';

const CakeShopHero = () => {
  const { data: homeDataList, isLoading } = useHomeData();
  const [currentSlide, setCurrentSlide] = useState(0);

  useEffect(() => {
    if (!homeDataList || homeDataList.length <= 1) return;
  ...
  if (isLoading || !homeDataList || homeDataList.length === 0) {
    return (
  ...
  return (
    <section className="relative h-screen overflow-hidden">
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center px-4">
        <div key={currentSlide} className="text-center space-y-6 max-w-4xl mx-auto">
